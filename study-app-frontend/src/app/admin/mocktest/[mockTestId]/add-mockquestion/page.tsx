...
export default function AddQuestionPage() {
  ...
  const mockTestId = Number(params.mockTestId);
   ...
   if (!mockTestId) {
    return <div className="text-red-600 text-center pt-20">Invalid Mock Test ID</div>;
  }
  const updateOption = (idx: number, data: Partial<MockOptionDto>) => {
    const updated = [...mockOptions];
    updated[idx] = { ...updated[idx], ...data };
    setOptions(updated);
  };

  const addOption = () => setOptions([...mockOptions, { optionText: '', isCorrect: false }]);

  const deleteOption = (idx: number) => {
    if (mockOptions.length <= 2) {
      setError('At least two options are required.');
      return;
    }

    const updated = mockOptions.filter((_, i) => i !== idx);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (mockOptions.filter((opt) => opt.isCorrect).length !== 1) {
    ...
    if (mockOptions.some((opt) => !opt.optionText.trim())) {
    ...
    const dto: MockQuestionCreateDto = { mockTestId, title, description, mockOptions};

    try {
      // Call your backend API via the existing service
      // This will POST to /Question/AddQuestion
      await mockQuestionService.addQuestion(dto);
