...
export default function AttemptResults({
        ...
        <div className="w-full max-w-4xl">
          <Card>
            <div className="text-center py-12">
              ...
              <Button
                onClick={() => router.replace('/student/exams/listing')}
              >
                Back to tests
              </Button>
            </div>
  ...
  const percentage =
    result.percentage ?? (total ? Math.round((correct / total) * 100) : 0);
        ...
        <div className="text-center mb-6">
          <div className="inline-flex items-center gap-4 bg-gradient-to-r from-yellow-200 to-blue-100 p-6 rounded-full shadow-lg mx-auto">
            ...
            <div className="text-left">
              <h2 className="text-3xl font-extrabold text-[#22418c]">
                Test Results
              </h2>
            </div>
          </div>
            ...
            <Card className="mb-4" bordered>
              ...
              <div className="space-y-4">
                {result.questions?.map((q: any, i: number) => {
                  const userOptionId =
                    q.userOptionId ?? q.userSelectedOptionId ?? null;
                  ...
                  const isUserCorrect =
                    userOptionId &&
                    correctOption &&
                    Number(userOptionId) === Number(correctOption.id);
                        ...
                        >
                          {(() => {
                            const userOpt = (q.options ?? []).find(
                              (o: any) =>
                                Number(o.id) === Number(userOptionId)
                            );
                            return userOpt
                              ? userOpt.optionText
                              : 'No answer';
